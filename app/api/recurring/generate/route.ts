...
export async function POST(req:Request){try{
 ...
 if(ie)throw ie;
 const next=new Date(sub.next_run_on); next.setMonth(next.getMonth()+step[sub.interval]);
 await db.from("recurring_invoices").update({next_run_on:next.toISOString().slice(0,10)}).eq("id",id).eq("org_id",m!.org_id);
